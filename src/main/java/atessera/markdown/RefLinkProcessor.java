...
public class RefLinkProcessor implements LinkProcessor {
	...
	if (text.startsWith("@@"))
	{
	            var ref = text.substring(2);
		    return LinkResult.replaceWith(new Reference(Reference.Type.PAGE, ref), position);
	}
        var ref = text.substring(1);
        return LinkResult.replaceWith(new Reference(Reference.Type.REGULAR, ref), position);
    }
}
